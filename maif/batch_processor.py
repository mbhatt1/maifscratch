...
class BatchProcessor(Generic[T, R]):
    ...
    async def _process_parallel(self, items: List[BatchItem[T]]) -> None:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._process_item_sync, item): item
                for item in items
            }
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error(f"Item {item.id} failed after {self.max_retries} attempts: {e}")
...
class StreamBatchProcessor(BatchProcessor[T, R]):
    ...
    async def _flush_batch(self):
        ...
        try:
            ...
            logger.info(f"Batch processed: {result.successful_items}/{result.total_items} successful")
        finally:
            self._processing = False
