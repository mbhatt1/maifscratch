...
class CertificatePathBuilder:
    ...
    def add_trust_anchor(self, cert_pem: str) -> bool:
        ...
        try:
            ...
            with self._lock:
                ...
                self.cert_cache[fingerprint] = self._extract_cert_info(cert, trust_level=3)
    ...
    def add_bridge_ca(self, cert_pem: str) -> bool:
        ...
        try:
            ...
            with self._lock:
                ...
                self.cert_cache[fingerprint] = self._extract_cert_info(cert, trust_level=2)
    ...
    def build_path(self, end_cert_pem: str, intermediate_certs: List[str] = None) -> Optional[List[CertificateInfo]]:
        """Build a certificate path from end certificate to trust anchor."""
        try:
            end_cert = load_pem_x509_certificate(end_cert_pem.encode(), default_backend())
            
            # Add intermediate certificates to cache
            if intermediate_certs:
                for int_cert_pem in intermediate_certs:
                    int_cert = load_pem_x509_certificate(int_cert_pem.encode(), default_backend())
                    fingerprint = int_cert.fingerprint(hashes.SHA256()).hex()
                    self.cert_cache[fingerprint] = self._extract_cert_info(int_cert, trust_level=2)
    ...
    def _build_path_recursive(self, cert: Any, current_path: List[CertificateInfo], 
                            visited: Set[str] = None) -> Optional[List[CertificateInfo]]:
        ...
        if fingerprint in self.bridge_cas:
            ...
            for cross_cert_fp in cert_info.cross_certificates:
                if cross_cert_fp in self.cert_cache:
                    ...
                    result = self._build_path_recursive(cross_cert, current_path, visited.copy())
                    if result:
                        return result
        ...
        for cached_fp, cached_info in self.cert_cache.items():
            if cached_info.subject == issuer_name.rfc4514_string():
                result = self._build_path_recursive(cached_info.certificate, current_path, visited.copy())
                if result:
                    return result
...
class MultiLevelPKIValidator:
    ...
    def validate_certificate_chain(self, end_cert_pem: str, 
                                 intermediate_certs: List[str] = None,
                                 check_revocation: bool = True) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            for i, cert_info in enumerate(path):
                ...
                if i < len(path) - 1:  # Not the last cert (root)
                    if not cert_info.is_ca:
                        result["errors"].append(f"Certificate {i} is not a CA but has issued certificates")
                        return False, result
                    ...
                    if cert_info.path_length_constraint is not None:
                        ...
                        if remaining_path > cert_info.path_length_constraint:
                            result["errors"].append(f"Path length constraint violated at certificate {i}")
                            return False, result
                
                # Check key usage
                if i < len(path) - 1 and "key_cert_sign" not in cert_info.key_usage:
                    result["errors"].append(f"Certificate {i} lacks key_cert_sign usage")
                    return False, result
                ...
                if check_revocation:
                    ...
                    if is_revoked:
                        result["errors"].append(f"Certificate {i} revoked at {revocation_time}")
                        return False, result
            ...
            for i in range(len(path) - 1):
                ...
                try:
                    ...
                    result["errors"].append(f"Signature verification failed at level {i}: {str(e)}")
                    return False, result
    ...
    def _check_revocation(self, cert_info: CertificateInfo) -> Tuple[bool, Optional[datetime]]:
...
class CrossCertificationManager:
    ...
    def add_cross_certification(self, domain1: str, domain2: str, 
                              cross_cert_pem: str) -> bool:
    ...
    def find_trust_path(self, cert_domain: str, target_domain: str) -> Optional[List[str]]:
...
def enhance_classified_security_with_multilevel_pki(cdp_instance):
    ...
    def validate_multilevel_pki(self, cert_pem: str, intermediate_certs: List[str] = None,
                               required_domain: str = None) -> Tuple[bool, Dict[str, Any]]:
        ...
        if required_domain:
            ...
            if cert_domain != required_domain:
                ...
                trust_path = cross_cert_manager.find_trust_path(cert_domain, required_domain)
                if not trust_path:
