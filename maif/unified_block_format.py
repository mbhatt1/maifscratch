...
class UnifiedBlockHeader:
    ...
    def to_bytes(self) -> bytes:
        ...
        prev_hash_bytes = (self.previous_hash or '').encode('ascii')[:64].ljust(64, b'\x00')
        block_hash_bytes = (self.block_hash or '').encode('ascii')[:64].ljust(64, b'\x00')
...
class UnifiedBlock:
    ...
    @classmethod
    def from_aws_format(cls, data: bytes, s3_metadata: Dict[str, str]) -> 'UnifiedBlock':
        """
...
class BlockFormatConverter:
    ...
    @staticmethod
    def legacy_to_unified(block_type: Union[str, int], data: bytes, 
                         metadata: Optional[Dict] = None) -> UnifiedBlock:
