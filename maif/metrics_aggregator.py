...
class MetricsAggregator:
    ...
    async def start(self):
        ...
        logger.info(f"MetricsAggregator started with {self.flush_interval}s flush interval")
        ...
        try:
            ...
            for agg in self.aggregated_metrics:
                ...
                base_metric = {
                    ...
                    'Dimensions': [
                        {'Name': k, 'Value': v}
                        for k, v in agg.dimensions.items()
                    ],
                ...
                metric_data.extend([
                    {**base_metric, 'Value': agg.sum, 'MetricName': f"{agg.name}_sum"},
                    {**base_metric, 'Value': agg.average, 'MetricName': f"{agg.name}_avg"},
                    {**base_metric, 'Value': agg.min, 'MetricName': f"{agg.name}_min"},
                    {**base_metric, 'Value': agg.max, 'MetricName': f"{agg.name}_max"},
                    {**base_metric, 'Value': agg.count, 'MetricName': f"{agg.name}_count"}
                ])
            ...
            self.last_flush_time = time.time()
            logger.info(f"Flushed {len(metric_data)} metrics in {self.total_batches_sent} batches")
...
class MAIFMetrics:
    ...
    def action_executed(self, agent_id: str, action_type: str, success: bool, duration_ms: float):
    ...
    def bedrock_request(self, model: str, input_tokens: int, output_tokens: int, duration_ms: float):
    ...
    def s3_operation(self, operation: str, bucket: str, size_bytes: int, duration_ms: float):
        """Record S3 operation."""
        self.aggregator.record_metric(
    ...
    def dynamodb_operation(self, operation: str, table: str, items: int, duration_ms: float):
        """Record DynamoDB operation."""
        self.aggregator.record_metric(
            f"dynamodb_{operation}_items",
            items,
