...
class SyncRateLimiter:
    ...
    def wait_if_needed(self, identifier: str = "global", cost: float = 1.0,
...
class HybridRateLimiter:
    ...
    def wait_sync(self, identifier: str = "global", cost: float = 1.0, 
                  max_wait: float = 60.0) -> float:
        """Synchronous wait for rate limit."""
        return self.sync_limiter.wait_if_needed(identifier, cost, max_wait)
    
    async def wait_async(self, identifier: str = "global", cost: float = 1.0,
                        max_wait: float = 60.0) -> float:
...
if __name__ == "__main__":
    ...
    adaptive_limiter = SyncAdaptiveRateLimiter(
        initial_rate=10,
        target_latency=0.5
    )
